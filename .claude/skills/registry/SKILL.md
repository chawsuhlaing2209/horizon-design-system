...
**Flag 7 — resolved, by deletion.** This flag originally read that `Semantic Tokens`
and `Semantic Tokens 2` were two undescribed text columns with near-identical names
and no recorded authority between them. Both `Semantic Tokens 2` and `Component Tokens`
have since been **deleted from the base**, and their ownership rows are removed above.
Components now carries 23 columns, not 25.

`Semantic Tokens` survives, still an undescribed single-line text field. The caution
